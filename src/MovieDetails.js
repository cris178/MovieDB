...
import {
  getTheMovieDBImageURL,
  getArtworkFromTheMovieDB,
  getCreditsFromTheMovieDB
} from "./themoviedb.js";
          ...
          <Paper className={classes.paper}>
            ...
            <List className={classes.list}>
              {this.state.cast.map(castMember => {
                const profilePic = castMember.profile_path
                  ? getTheMovieDBImageURL(castMember.profile_path)
                  : castMember.profilePic;
                return (
                  <ListItem>
                    <Avatar
                      alt={castMember.name}
  ...
  componentDidMount() {
    ...
    getIMDBTitleInfo(id, titleInfo => {
      this.setState({
        ...
        */
        primaryTitle: titleInfo.primaryTitle,
        runtimeMinutes: titleInfo.runtimeMinutes,
        tconst: titleInfo.tconst,
        genres: titleInfo.genres,
        titleType: titleInfo.titleType,
        endYear: titleInfo.endYear,
        startYear: titleInfo.startYear
      });
    });
    ...
    getArtworkFromTheMovieDB(tconst)
      .then(res => {
        ...
        if (this.state.artWorkObject && this.state.artWorkObject.length > 0) {
          this.setState({
            poster: getTheMovieDBImageURL(
              this.state.artWorkObject[0].poster_path
            ),
      ...
      .then(res => {
        getCreditsFromTheMovieDB(this.state.artWorkId).then(res => {
          ...
          /* Todo3: add a this.setState call to update the crew and cast properties, from the
           castAndCrew variable holding the request result.  From the API documentation
           at https://developers.themoviedb.org/3/movies/get-movie-credits we can see
           that the API response has a cast field and a crew field, which have the values
           we need.  See the this.setState call above for an example of how to update state
           fields.
        */
          this.setState({
